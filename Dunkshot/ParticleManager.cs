...
namespace Dunkshot
{
    public class ParticleManager<T>
    {
        ...
        public void DrawPrepare(SpriteBatch spriteBatch, RenderTarget2D renderTarget)
        {
        ...
        {
            /* Note that these fireworks are the same, only slightly less beautiful..
            spriteBatch.End();
            spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Additive);
            for (int i = 0; i < particleList.Count; i++)
            {
                ...
                Vector2 origin = new Vector2(particle.Texture.Width / 2, particle.Texture.Height / 2);
                spriteBatch.Draw(particle.Texture, new Vector2(particle.Position.X+300, particle.Position.Y), null, particle.Tint, particle.Orientation, origin, particle.Scale, 0, 0);
            }
            spriteBatch.End();
            spriteBatch.Begin();
            */

            spriteBatch.Draw(renderTarget, new Rectangle(0, 0, 1920, 1080), new Color(255,255,255,90));
        }
